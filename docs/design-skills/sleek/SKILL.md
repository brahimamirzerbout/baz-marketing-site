...
## Style
A sleek, modern interface with bold typography, deep navy tones, pill-shaped controls, and refined UI elements that create a sophisticated, premium experience


## Before Writing Any Code

1. **Read every module that applies.** For a landing page, read at minimum: `layout.md`, `typography.md`, `colors.md`, `buttons.md`, `cards.md`, `shadows.md`, `radius.md`, `borders.md`. Do NOT write any UI code until you have loaded all relevant modules.

## Critical Rules

- **Tokens are AGNOSTIC, NOT framework classes:** The tokens defined in the `.md` files (like `neutral-primary-soft`, `heading`, `border-default`) are agnostic design system tokens, NOT literal CSS framework classes. Do not blindly convert token names to class names — you must implement the mapping for your specific stack (CSS custom properties, preprocessor variables, design token tooling, etc.).
