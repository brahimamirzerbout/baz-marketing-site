...
## Style
A refined enterprise interface with sharp geometric edges, structured layouts, authoritative serif headings (Playfair Display) paired with clean sans-serif body text (Inter) that conveys professional clarity and precision. Brand color: #6733FF (vibrant purple). No border radius — all corners are sharp and square for a corporate, editorial feel.

## Design Principles

- **No border radius:** All UI elements use 0px radius (sharp corners) except pills, avatars, and toggles which use 9999px (full). Never round corners on buttons, cards, inputs, modals, alerts, badges, tabs, or containers.
- **Font family — dual font system:** **Playfair Display (serif)** is used exclusively for headings (h1–h6). **Inter (sans-serif)** is used for everything else — paragraphs, body copy, buttons, labels, navigation, badges, inputs, and all other UI text. Both fonts must be loaded at app level. Set Inter as the body font and apply Playfair Display explicitly to heading elements.
- **Brand color:** #6733FF is the primary brand color. All brand tokens (backgrounds, text, borders) derive from this purple.
- **Enterprise aesthetic:** Clean, flat surfaces. No decorative glint effects on buttons. Minimal shadows. Crisp borders. Professional restraint.
- **No decorative flourishes:** Avoid gradient meshes, noise textures, grain overlays, or heavy decorative patterns. Use solid colors, clean borders, and structured whitespace.
