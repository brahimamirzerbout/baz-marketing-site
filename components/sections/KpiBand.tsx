      ...
      <div className="grid lg:grid-cols-12 gap-10 items-end">
        ...
        <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-3 gap-px bg-background/10 rounded-2xl overflow-hidden border border-border/50">
          {[
            ...
            <div key={s.l} className="bg-primary text-foreground p-6 md:p-8 dark:bg-primary/90 dark:text-foreground">
              <p className="font-display text-4xl md:text-5xl font-medium tracking-[-0.03em] text-foreground dark:text-foreground">{s.v}</p>
              <p className="mt-3 font-mono uppercase tracking-[0.18em] text-[11px] text-muted-foreground dark:text-muted-foreground">{s.l}</p>
              <p className="mt-2 text-sm text-muted-foreground/60 dark:text-muted-foreground">{s.s}</p>
            </div>
          ))}
        </div>
      </div>
