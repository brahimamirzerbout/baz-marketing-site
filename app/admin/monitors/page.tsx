    ...
    <div className="container mx-auto py-10 md:py-16">
      <header className="flex flex-wrap items-end justify-between gap-6 mb-8">
        <div>
          <p className="font-mono uppercase tracking-[0.18em] text-[11px] text-accent mb-2">
            BAZ Monitors · /admin/monitors
          </p>
          <h1 className="font-display text-display-xl font-medium tracking-[-0.035em]">
            Efficiency, at a glance.
          </h1>
          <p className="mt-3 text-muted-foreground max-w-xl">
            Live health of the BAZ build, API, lead funnel, AI spend, and the public site&apos;s
            Core Web Vitals.
          </p>
        </div>
        <div className="text-right">
          ...
          <p className="mt-1 inline-flex items-center gap-2 text-xs font-mono">
            <span
              className={`w-2 h-2 rounded-full ${live ? "bg-success animate-pulse-dot" : "bg-warning"}`}
            />
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Tile title="Build" subtitle="Next.js + lint + typecheck" data={data.build}>
          ...
          <KV
            k="Build time"
            v={data.build.buildMs ? `${(data.build.buildMs / 1000).toFixed(1)}s` : "n/a"}
          />
          <KV
            k="First-load JS"
            v={data.build.bundleKb ? `${data.build.bundleKb.toFixed(1)} kB` : "n/a"}
          />
        </Tile>
        ...
        <Tile title="API" subtitle={`v${data.api.version}`} data={data.api}>
          ...
          <KV
            k="AI provider"
            v={data.api.provider}
            status={data.api.provider === "not configured" ? "warn" : "ok"}
          />
          <KV
            k="Stripe mode"
            v={data.api.stripeMode}
            status={data.api.stripeMode === "not configured" ? "warn" : "ok"}
          />
        </Tile>
        ...
        <Tile title="Lead funnel" subtitle="Captured via contact form" data={data.leads}>
          ...
          <KV
            k="Trend (7d)"
            v={`${data.leads.trendPct >= 0 ? "+" : ""}${data.leads.trendPct}%`}
            status={data.leads.trendPct >= 0 ? "ok" : "warn"}
          />
        </Tile>
        ...
        <Tile title="Tips" subtitle="What good looks like" data={{ status: "ok" }}>
          <ul className="text-sm text-foreground space-y-2 list-disc pl-5">
            ...
            <li>
              LCP &lt; 2.5s, CLS &lt; 0.1, INP &lt; 200ms — Google&apos;s &quot;good&quot;
              thresholds.
            </li>
          </ul>
        </Tile>
  ...
  return (
    <section className="rounded-2xl border border-border bg-background p-5 md:p-6">
      <header className="flex items-center justify-between mb-4">
        ...
        <span
          className={`px-2.5 py-1 rounded-full border text-[11px] font-mono uppercase tracking-[0.14em] ${tone}`}
        >
          {data.status}
        </span>
      </header>
...
function KV({ k, v, status, mono }: { k: string; v: string; status?: Status; mono?: boolean }) {
  ...
  return (
    <div>
      <dt className="text-[11px] font-mono uppercase tracking-[0.14em] text-muted-foreground/60">
        {k}
      </dt>
      <dd
        className={`text-sm ${tone} ${mono ? "font-mono text-[12.5px] truncate" : "font-medium"}`}
      >
        {v}
      </dd>
    </div>
...
function lcp(ms: number) {
  return ms <= 2500 ? "ok" : ms <= 4000 ? "warn" : "down";
}
function cls(v: number) {
  return v <= 0.1 ? "ok" : v <= 0.25 ? "warn" : "down";
}
function inp(ms: number) {
  return ms <= 200 ? "ok" : ms <= 500 ? "warn" : "down";
}
function statusOf(s: string): Status {
  return s === "ok" ? "ok" : s === "down" ? "down" : "warn";
}
