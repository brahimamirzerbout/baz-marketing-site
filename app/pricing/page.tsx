    ...
    <>
      <Section tone="paper" size="lg">
        ...
        <div className="max-w-4xl">
          ...
          <p className="mt-6 text-lg md:text-2xl text-muted-foreground leading-relaxed max-w-3xl">
            Project-based or retainer. Pick the engagement that matches the work, not the work that
            matches the engagement.
          </p>
        </div>
          ...
          <div className="grid md:grid-cols-3 gap-6 items-center">
            <div className="md:col-span-2">
              ...
              <p className="mt-2 text-muted-foreground text-sm">
                From kickoff, the first shippable artifact is live in your Hub within 14 calendar
                days. We&apos;ve never paid out.
              </p>
            </div>
            <div className="md:col-span-1 text-center md:text-right">
              <div className="font-display text-6xl font-medium tracking-[-0.04em] text-accent">
                14d
              </div>
      ...
      <Section tone="white" size="lg">
          ...
          {tiers.map((tier, i) => (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <span
                  className={`font-mono text-[10px] uppercase tracking-[0.18em] ${tier.featured ? "text-muted-foreground" : "text-muted-foreground"}`}
                >
              ...
              <p
                className={`text-[15px] leading-relaxed ${tier.featured ? "text-muted-foreground" : "text-muted-foreground"}`}
              >
                {tier.tagline}
              </p>

              <div
                className={`mt-8 pb-6 border-b ${tier.featured ? "border-border/60" : "border-border"}`}
              >
                <p
                  className={`font-display text-3xl md:text-4xl font-medium tracking-[-0.03em] ${tier.featured ? "text-foreground" : "text-foreground"}`}
                >
                  {formatPrice(tier.monthly.min, tier.monthly.max)}
                </p>
                <p
                  className={`mt-1 text-xs font-mono uppercase tracking-[0.15em] ${tier.featured ? "text-muted-foreground" : "text-muted-foreground"}`}
                >
                  {tier.monthly.label}
                </p>
                <p
                  className={`mt-3 text-xs ${tier.featured ? "text-muted-foreground" : "text-muted-foreground"}`}
                >
                  {tier.cadence}
                </p>
              </div>
              ...
              <p
                className={`mt-6 text-sm ${tier.featured ? "text-muted-foreground" : "text-foreground"} leading-relaxed`}
              >
                {tier.description}
              </p>
              ...
              <div className="mt-6 flex-1">
                <p
                  className={`text-[11px] font-mono uppercase tracking-[0.18em] mb-3 ${tier.featured ? "text-muted-foreground" : "text-muted-foreground"}`}
                >
                  What&apos;s included
                </p>
              ...
              <div
                className={`mt-6 pt-4 border-t ${tier.featured ? "border-border/60" : "border-border"}`}
              >
                <p
                  className={`text-[11px] font-mono uppercase tracking-[0.18em] mb-2 ${tier.featured ? "text-muted-foreground" : "text-muted-foreground"}`}
                >
                  Best for
                </p>
                <ul className="space-y-1.5">
                  {tier.bestFor.map((b) => (
                    <li
                      key={b}
                      className={`text-sm ${tier.featured ? "text-muted-foreground" : "text-muted-foreground"}`}
                    >
                      {b}
                    </li>
                  ))}
        ...
        <p className="mt-10 text-center text-sm text-muted-foreground max-w-2xl mx-auto">
          Prices shown are USD monthly ranges. Final quote depends on scope, seniority mix, and
          engagement length. Book a call for a specific number — we don&apos;t do mystery pricing.
        </p>
      </Section>
      ...
      <Section tone="paper" size="lg">
        <div className="grid lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-7 grid sm:grid-cols-2 gap-6">
            <div>
              <h3 className="font-display text-lg font-medium tracking-[-0.02em] mb-2">
                No junior pods
              </h3>
              <p className="text-sm text-muted-foreground leading-relaxed">
                The senior who pitches the work ships the work. You won&apos;t be handed off to a
                junior team after the contract is signed.
              </p>
            </div>
            <div>
              <h3 className="font-display text-lg font-medium tracking-[-0.02em] mb-2">
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">
                You see what&apos;s shipping next, this week. No status decks, no surprises.
              </p>
            </div>
            <div>
              <h3 className="font-display text-lg font-medium tracking-[-0.02em] mb-2">
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">
                Every engagement has a checkpoint. We tell you honestly if the work is paying off —
                and when it isn&apos;t.
              </p>
            </div>
            <div>
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">
                Retainers are quarterly. We don&apos;t ask for annual commitments because we
                don&apos;t need to.
              </p>
            </div>
          </div>
