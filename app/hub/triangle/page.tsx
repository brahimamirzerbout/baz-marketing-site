...
import { Activity, Zap, Target, TrendingUp, Play, AlertCircle, CheckCircle2, Circle } from 'lucide-react';
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <ScrollReveal y={12}>
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Marketing ↔ Sales ↔ Nova. Runs every 60s.</p>
          </div>
          <button
            onClick={runNow}
            disabled={running}
      ...
      <ScrollReveal y={16} delay={0.1}>
        <div className="rounded-2xl bg-primary border border-ink-800 p-8 flex justify-center">
          <svg viewBox="0 0 400 400" className="w-full max-w-sm" aria-hidden>
            ...
            <text x="200" y="46" textAnchor="middle" fill="#fff" fontSize="14" fontFamily="serif">Marketing</text>
            <text x="360" y="326" textAnchor="middle" fill="#fff" fontSize="14" fontFamily="serif">Sales</text>
            <text x="40" y="326" textAnchor="middle" fill="#fff" fontSize="14" fontFamily="serif">Nova</text>
            <path d="M 175 75 C 230 150, 320 220, 345 285" fill="none" stroke="#ff5b1f" strokeWidth="1.5" strokeDasharray="4 4" />
            <path d="M 55 285 C 80 220, 170 150, 225 75" fill="none" stroke="#c9a961" strokeWidth="1.5" strokeDasharray="4 4" />
            <path d="M 200 90 C 200 200, 200 200, 200 270" fill="none" stroke="#3ddc97" strokeWidth="1.5" strokeDasharray="4 4" />
            <text x="280" y="180" fill="#c9a961" fontSize="10" fontFamily="monospace">score → route</text>
            <text x="120" y="180" fill="#3ddc97" fontSize="10" fontFamily="monospace">reason → act</text>
            <text x="208" y="200" fill="#ff5b1f" fontSize="10" fontFamily="monospace">step</text>
          </svg>
        </div>
      </ScrollReveal>
      ...
      <ScrollReveal y={20} delay={0.2}>
        <div className="grid md:grid-cols-3 gap-4">
          {[
            { t: '1. Score', d: 'Touchpoint weight + recency + channel mix → 0–100 lead score.', color: '#ff5b1f' },
            { t: '2. Route', d: 'Score ≥ 50 → enroll into least-loaded active sequence.', color: '#c9a961' },
            { t: '3. Step', d: 'Each enrollment advances daily. Reply pauses. Booking promotes.', color: '#3ddc97' },
            { t: '4. Close', d: 'Deals with prob ≥ 80 auto-close as won. Stale → auto-lost at 45d.', color: '#4f7cff' },
            { t: '5. Learn', d: 'Wins feed back into Nova. Tomorrow is smarter than today.', color: '#7c3aed' },
            { t: '6. Report', d: 'Pipeline value, velocity, wins/day — one cockpit, no spreadsheets.', color: '#f9a01f' },
          ].map((s, i) => (
            <motion.div
              key={s.t}
        ...
        <ScrollReveal y={16} delay={0.3}>
          <div className="rounded-2xl bg-primary border border-ink-800 p-5">
            <div className="flex items-center gap-2 mb-4">
              <span className={`inline-block w-2 h-2 rounded-full ${h.ok ? 'bg-emerald-500' : 'bg-amber-500'} animate-pulse`} />
            ...
            <div className="grid grid-cols-4 gap-3 text-center">
              {[
                ...
                { l: 'Pipeline', v: `$${Math.round(h.pipeline_value ?? 0).toLocaleString()}`, c: 'text-emerald-400' },
