...
export default function NovaPage() {
  const [messages, setMessages] = useState<Message[]>([
    { role: 'nova', text: "I'm Nova — the brain above the Hub. I can read your CRM, campaigns, lexicon, library, and trends. Ask me anything about your business.", at: Date.now() },
  ]);
  ...
  async function ask(q?: string) {
    ...
    try {
      ...
      if (data && !data.error) {
        ...
        setMessages(m => [...m, { role: 'nova', text: "Couldn't answer that right now. Make sure the Hub is running.", at: Date.now() }]);
      }
    } catch {
      setMessages(m => [...m, { role: 'nova', text: "Hub not reachable. Start the Marketing Hub on port 3010.", at: Date.now() }]);
    }
    setBusy(false);
  }
    ...
    <div className="max-w-3xl mx-auto space-y-4">
      ...
      <ScrollReveal y={12}>
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl grid place-items-center" style={{ background: 'linear-gradient(135deg, #7c3aed, #ec4899)' }}>
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground/60">The brain above the tools. Ask anything.</p>
          </div>
        </div>
      </ScrollReveal>
          ...
          <div className="p-4 space-y-3 max-h-[55vh] overflow-y-auto">
            {messages.map((m, i) => (
              <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'justify-end' : ''}`}>
                {m.role === 'nova' && (
                  <div className="w-8 h-8 rounded-full grid place-items-center flex-shrink-0" style={{ background: 'linear-gradient(135deg, #7c3aed, #ec4899)' }}>
                ...
                <div className={`max-w-[80%] rounded-2xl p-3 ${
                  ...
                  {m.role === 'nova' && (m.sources?.length || m.actions?.length || m.followups?.length) && (
                    <div className="mt-2 space-y-2 text-xs">
                      {m.confidence != null && (
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex-1 h-1 rounded-full bg-muted overflow-hidden max-w-[100px]">
                            <div className="h-full bg-emerald-400" style={{ width: `${m.confidence}%` }} />
                          </div>
                      ...
                      {m.sources && m.sources.length > 0 && (
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {m.sources.map(s => <span key={s} className="px-1.5 py-0.5 rounded bg-muted text-muted-foreground">{s}</span>)}
                        </div>
                      )}
                      {m.actions && m.actions.length > 0 && (
                        <div>
                          <div className="text-muted-foreground/60">Actions:</div>
                          <ul className="mt-1 space-y-1">{m.actions.map((a, j) => <li key={j}>• {a}</li>)}</ul>
                        </div>
                      )}
                      {m.followups && m.followups.length > 0 && (
                        <div className="flex items-center gap-1.5 flex-wrap pt-1">
                          ...
                          {m.followups.map((f, j) => (
                            <button key={j} onClick={() => ask(f)} className="px-2 py-0.5 rounded-full bg-violet-500/20 text-violet-200 hover:bg-violet-500/30 text-[10px]">
                              {f}
                            </button>
                          ))}
                        </div>
              ...
              <div className="flex gap-3">
                ...
                <div className="rounded-2xl p-3 bg-primary/90 text-foreground text-sm">
                  <span className="inline-flex gap-1">
                    {[0, 0.2, 0.4].map((d, i) => (
                      <span key={i} className="w-1.5 h-1.5 rounded-full bg-muted/60 animate-pulse" style={{ animationDelay: `${d}s` }} />
                    ))}
                  </span>
                </div>
              </div>
