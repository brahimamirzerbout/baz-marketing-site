...
const PAGE_META: Record<string, { title: string; desc: string }> = {
  '/hub/sequences': { title: 'Sequences', desc: 'Sales cadences that learn — pause on reply, escalate on open' },
  '/hub/attribution': { title: 'Attribution', desc: 'Multi-touch revenue attribution across every channel' },
  '/hub/analytics': { title: 'Analytics', desc: 'Tracking, dashboards, and exec reports you actually open' },
  '/hub/seo': { title: 'SEO Toolkit', desc: 'Technical SEO, keywords, content gaps, SERP tracking' },
  '/hub/emails': { title: 'Email Builder', desc: 'Design, build, and send email campaigns' },
  '/hub/landing-pages': { title: 'Landing Pages', desc: 'Build, publish, and A/B test landing pages' },
  ...
  '/hub/orchestrator': { title: 'Orchestrator', desc: 'Campaign orchestration and task management' },
...
export default function HubEmbedPage() {
  ...
  const meta = PAGE_META[pathname] || { title: hubPath.split('/').pop() || 'Hub', desc: 'Marketing Hub page' };
    ...
    <div className="max-w-6xl mx-auto">
      ...
      >
        <div>
          ...
          <h1 className="font-display text-2xl md:text-3xl font-medium text-foreground">{meta.title}</h1>
