...
import {
  Rocket, Activity, Brain, Target, Zap, GitBranch,
  Newspaper, BarChart3, Users, CreditCard, ArrowRight, TrendingUp
} from 'lucide-react';
...
const QUICK_LINKS = [
  { href: '/hub/cockpit', label: 'Cockpit', desc: 'Pipeline velocity, wins, forecast', icon: Rocket, color: '#ff3b2f' },
  { href: '/hub/triangle', label: 'Triangle Loop', desc: 'Autonomous scoring → routing → closing', icon: Activity, color: '#3ddc97' },
  { href: '/hub/nova', label: 'Nova AI', desc: 'Ask anything about your business', icon: Brain, color: '#7c3aed' },
  { href: '/hub/sequences', label: 'Sequences', desc: 'Sales cadences that learn', icon: Zap, color: '#f9a01f' },
  { href: '/hub/attribution', label: 'Attribution', desc: 'Multi-touch revenue attribution', icon: GitBranch, color: '#4f7cff' },
  { href: '/hub/wire', label: 'The Wire', desc: 'Industry intelligence, scored', icon: Newspaper, color: '#e11d48' },
];
...
export default function HubDashboard() {
    ...
    async function load() {
      try {
        const [h, d] = await Promise.all([
          ...
          fetch(`${HUB_URL}/api/deals`).then(r => r.ok ? r.json() : null).catch(() => null),
        ]);
        setHealth(h);
        setDeals(d);
      } catch {}
  ...
  const stats = [
    { label: 'Pipeline value', value: health ? `$${Math.round(health.pipeline_value || 0).toLocaleString()}` : '—', icon: Target, color: '#ff3b2f' },
    { label: 'Active sequences', value: health?.enrollments_active ?? '—', icon: Zap, color: '#f9a01f' },
    { label: 'Wins (7d)', value: health?.recent_wins_7d ?? '—', icon: TrendingUp, color: '#3ddc97' },
    { label: 'Velocity', value: health ? `${(health.triangle_velocity || 0).toFixed(2)}/day` : '—', icon: Activity, color: '#4f7cff' },
  ];
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-muted-foreground text-sm max-w-2xl">
              Marketing, sales, and finance — running in a closed loop. Every contact scored.
              Every cadence stepped. Every outcome learned from.
            </p>
          </div>
                ...
                >
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="w-10 h-10 rounded-xl grid place-items-center" style={{ background: `${link.color}20` }}>
                    ...
                    <h3 className="font-display text-lg font-medium text-foreground">{link.label}</h3>
      ...
      <ScrollReveal y={16} delay={0.4}>
        <div className="rounded-2xl bg-gradient-to-br from-ink-900 to-ink-950 border border-border p-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground/60 mt-1">All 110+ pages, complete CRM, finance, and automation suite.</p>
          </div>
          <a
            href={HUB_URL}
            target="_blank"
