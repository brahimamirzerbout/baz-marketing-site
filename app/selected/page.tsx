      ...
      <section className="relative overflow-hidden border-b border-border">
        ...
        <div className="relative z-10 mx-auto max-w-[1400px] px-6 pt-32 pb-20 lg:px-10 lg:pt-40 lg:pb-28">
          ...
          <div className="mt-10 flex flex-wrap gap-3">
            <a href={site.bookOrMailto} className="inline-flex items-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-medium text-primary-foreground transition-transform hover:-translate-y-px">
              Apply for selection →
            </a>
            <a href="#criteria" className="inline-flex items-center gap-2 rounded-full border border-border px-6 py-3 text-sm font-medium text-ink-900 transition-colors hover:bg-ink-100">
              See the criteria
            </a>
          </div>
        </div>
      ...
      <section className="border-b border-border">
        <div className="mx-auto grid max-w-[1400px] grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <div key={s.l} className={`relative px-6 py-10 lg:px-10 lg:py-14 ${i > 0 ? "border-l border-border" : ""} ${i < 2 ? "border-b border-border lg:border-b-0" : ""}`}>
          ...
          <div className="glass grid gap-10 p-8 lg:grid-cols-[1.3fr_1fr] lg:p-14">
            <div>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <a href={site.bookOrMailto} className="inline-flex items-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-medium text-primary-foreground">
                  Apply for selection <span aria-hidden>→</span>
                </a>
                <a href={`mailto:${site.email}`} className="inline-flex items-center gap-2 rounded-full border border-border px-6 py-3 text-sm font-medium text-ink-900 hover:bg-ink-100">
                  {site.email}
                </a>
              </div>
            </div>
            <ul className="flex flex-col gap-4 lg:border-l lg:border-border lg:pl-10">
              {["Reply within 24 hours", "Selection, not a pitch", "Confidential — NDA on request", "3 slots — closes when filled"].map((item) => (
