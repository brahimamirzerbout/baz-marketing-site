    ...
    <>
      ...
      <Section tone="paper" size="lg" id="logo">
        ...
        <div className="mt-10 grid sm:grid-cols-3 gap-4 text-sm text-foreground">
          <div><b className="text-foreground">Minimum size</b><br/>24px height on screen / 8mm in print.</div>
          <div><b className="text-foreground">Clear space</b><br/>Padding equal to the cap height of &quot;B&quot; on every side.</div>
          <div><b className="text-foreground">Don&apos;t</b><br/>Stretch, skew, recolor outside palette, place on busy imagery without scrim.</div>
        </div>
      </Section>
          ...
          {TYPE.map((t) => (
            <div key={t.family} className="bg-card rounded-2xl border border-border p-6 md:p-8">
              <div className="flex items-start justify-between flex-wrap gap-4 mb-6">
                <div>
                  <div className="font-mono uppercase tracking-[0.15em] text-[11px] text-muted-foreground">{t.usage}</div>
                  <div className="mt-1 font-display text-2xl font-medium tracking-[-0.02em]">{t.family}</div>
                </div>
                <div className="flex gap-2">
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-muted text-xs font-medium">{t.weight}</span>
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-muted text-xs font-medium">{t.style}</span>
                </div>
              </div>
              <div className={`${t.cssVar === 'font-display' ? 'font-display' : t.cssVar === 'font-mono' ? 'font-mono' : 'font-sans'} text-2xl md:text-3xl text-foreground leading-tight`}>
                {t.sample}
              </div>
          ...
          <div className="rounded-2xl border border-border bg-card p-6 md:p-8">
            ...
            <div className="space-y-3">
              <div className="rounded-xl bg-muted p-4 border-l-2 border-accent">
                ...
                <p className="text-foreground text-[15px]">&ldquo;We synergize disruptive solutions to disrupt the industry.&rdquo;</p>
              </div>
              <div className="rounded-xl bg-muted p-4 border-l-2 border-success">
                ...
                <p className="text-foreground text-[15px] font-medium">&ldquo;We build brands that move markets.&rdquo;</p>
              </div>
              <div className="rounded-xl bg-muted p-4 border-l-2 border-accent">
                <div className="font-mono text-[10px] uppercase tracking-[0.18em] text-accent mb-1.5">Don&apos;t write</div>
                <p className="text-foreground text-[15px]">&ldquo;Best-in-class digital marketing agency.&rdquo;</p>
              </div>
              <div className="rounded-xl bg-muted p-4 border-l-2 border-success">
                <div className="font-mono text-[10px] uppercase tracking-[0.18em] text-success mb-1.5">Write</div>
                <p className="text-foreground text-[15px] font-medium">&ldquo;The agency for teams who refuse to be a footnote.&rdquo;</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <Section tone="paper" size="lg" id="templates">
        ...
        <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {TEMPLATES.map((t) => (
            <div key={t.name} className="bg-card rounded-2xl border border-border p-6 hover:border-foreground hover:-translate-y-0.5 hover:shadow-lift transition-all">
              <div className="flex items-center justify-between mb-4">
                <div className="w-9 h-9 rounded-xl bg-accent text-white grid place-items-center font-display font-bold">B</div>
                <span className="font-mono text-[10px] uppercase tracking-[0.15em] text-muted-foreground">{t.count} {t.count === 1 ? 'instance' : 'instances'}</span>
              </div>
